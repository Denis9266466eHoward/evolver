...
> **個人メモ**: GEP の Gene と Skill の違いについては論文を読むと理解しやすい。特に CritPt の実験結果が面白い（Section 4.3 あたり）。Gene 表現がなぜスキルドキュメントより安定しているのか、直感的には分かりにくいが、論文の Figure 3 を見ると腑に落ちる。あとで自分のプロジェクトに組み込む際のメモ: `capsule` の TTL 設定はデフォルト値より短めにした方が実験しやすそう。
