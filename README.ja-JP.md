...
> **個人メモ**: GEP の Gene と Skill の違いについては論文を読むと理解しやすい。特に CritPt の実験結果が面白い。Gene 表現がなぜ Skill ドキュメントより安定しているのか、Section 4 の ablation study を見ると直感的に掴める。自分の用途（コード生成タスクへの適用）では Gene の粒度設定が重要そうなので、そこを重点的に試していく予定。
